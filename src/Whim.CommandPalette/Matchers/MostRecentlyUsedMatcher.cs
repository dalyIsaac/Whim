...
public class MostRecentlyUsedMatcher<T> : IMatcher<T>
{
	...
	/// </summary>
	public ICollection<IPaletteItem<T>> Match(string query, IReadOnlyCollection<IPaletteItem<T>> inputItems)
	{
		...
		if (matchCount == 0)
		{
			...
			{
				return Array.Empty<IPaletteItem<T>>();
			}
		...
		IPaletteItem<T>[] matchedItems = new IPaletteItem<T>[matchCount];
	...
	internal int GetFilteredItems(string query, IReadOnlyCollection<IPaletteItem<T>> items, MatcherItem<T>[] matches)
	{
		int matchCount = 0;

		// Get the matches for the query.
		foreach (IPaletteItem<T> item in items)
		{
	...
	/// </summary>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	internal int GetMostRecentlyUsedItems(IReadOnlyCollection<IPaletteItem<T>> items, MatcherItem<T>[] matches)
	{
		int matchCount = 0;
