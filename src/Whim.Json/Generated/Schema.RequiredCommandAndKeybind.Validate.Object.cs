        ...
        private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
        {
            ...
            foreach (JsonObjectProperty property in this.EnumerateObject())
            {
                if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
                {
                    ...
                    if ((this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.CommandUtf8)) || (!this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.Command)))
                    {
                        foundCommand = true;
                    }
                    else if ((this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.KeybindUtf8)) || (!this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.Keybind)))
                    {
                        foundKeybind = true;
                    }
                }
            ...
            if (!foundCommand)
            {
                if (level >= ValidationLevel.Detailed)
                {
                    result = result.WithResult(isValid: false, $"6.5.3. required - required property \"command\" not present.");
                }
                else if (level >= ValidationLevel.Basic)
                {
            ...
            if (!foundKeybind)
            {
                if (level >= ValidationLevel.Detailed)
                {
                    result = result.WithResult(isValid: false, $"6.5.3. required - required property \"keybind\" not present.");
                }
                else if (level >= ValidationLevel.Basic)
                {
                    result = result.WithResult(isValid: false, "6.5.3. required - required property not present.");
