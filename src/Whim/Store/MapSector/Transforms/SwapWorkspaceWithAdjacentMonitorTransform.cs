	...
	: Transform
{
	internal override Result<Unit> Execute(IContext ctx, IInternalContext internalCtx, MutableRootSector rootSector)
	{
		...
		Result<IMonitor> currentMonitorResult = ctx.Store.Pick(Pickers.PickMonitorForWorkspace(workspaceId));
		...
		Result<IWorkspace> nextWorkspaceResult = ctx.Store.Pick(Pickers.PickWorkspaceForMonitor(nextMonitor.Handle));
		if (!nextWorkspaceResult.TryGet(out IWorkspace nextWorkspace))
		{
			return Result.FromException<Unit>(nextWorkspaceResult.Error!);
		}

		ctx.Store.Dispatch(new ActivateWorkspaceTransform(nextWorkspace.Id, currentMonitor.Handle));
