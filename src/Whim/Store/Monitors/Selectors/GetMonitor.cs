...
public record GetActiveMonitor() : Selector<IMonitor>()
{
	internal override IMonitor Execute(IContext ctx, IInternalContext internalCtx, RootSlice root) =>
		root.MonitorSlice.Monitors[root.MonitorSlice.ActiveMonitorIndex];
}
...
{
	internal override IMonitor Execute(IContext ctx, IInternalContext internalCtx, RootSlice root) =>
		root.MonitorSlice.Monitors[root.MonitorSlice.PrimaryMonitorIndex];
}
...
{
	internal override IMonitor Execute(IContext ctx, IInternalContext internalCtx, RootSlice root) =>
		root.MonitorSlice.Monitors[root.MonitorSlice.LastWhimActiveMonitorIndex];
}
...
{
	internal override IReadOnlyList<IMonitor> Execute(IContext ctx, IInternalContext internalCtx, RootSlice root) =>
		root.MonitorSlice.Monitors;
}
