...
{
	public static WhimError MonitorNotFound(HMONITOR handle) => new($"Monitor with handle {handle} not found.");

	public static WhimError WindowNotFound(HWND handle) => new($"Window with handle {handle} not found.");

	public static WhimError WindowNotFoundInWorkspace(HWND handle, WorkspaceId workspaceId) =>
		new($"Window with handle {handle} not found in workspace {workspaceId}.");

	public static WhimError WorkspaceNotFound(WorkspaceId workspaceId) => new($"Workspace {workspaceId} not found.");

	public static WhimError NoMonitorFoundAtPoint(IPoint<int> point) => new($"No monitor found at point {point}.");
	...
	public static WhimError InvalidMonitorIndex(int monitorIndex) => new($"No monitor found at index {monitorIndex}.");
}
